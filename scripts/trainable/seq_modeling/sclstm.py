...
from helpers import VectorType
...
def model_predictions(model, data, vocab, DEVICE, BATCH_SIZE=16):
    ...
    for batch_id, (batch_clean_sentences,batch_corrupt_sentences) in enumerate(data_iter):
        ...
        batch_idxs, batch_lengths_ = sclstm_tokenize(batch_corrupt_sentences, vocab, vector_type=VectorType.Default)
...
if __name__=="__main__":
    ...
    if START_EPOCH>0:
        ...
        if FINETUNE!="":
            ...
            if TRAIN_NOISE_TYPE=="word":
                ...
                # corpus_100_bad.txt
                train_data = load_data(TRAIN_TEST_FILE_PATH, "train_good.txt", "train_bad.txt")
    ...
    #     vocab = get_tokens([i[0] for i in train_data + valid_data],
    #                        keep_simple=False,
    ...
    if START_EPOCH>0:
        ...
        for epoch_id in range(START_EPOCH,N_EPOCHS+1):
            ...
            for batch_id, (batch_labels,batch_sentences) in enumerate(train_data_iter):
                ...
                batch_idxs, batch_lengths_ = sclstm_tokenize(batch_sentences, vocab, vector_type=VectorType.Default)
                assert (batch_lengths_==batch_lengths).all()==True
                batch_idxs = [batch_idxs_.to(DEVICE) for batch_idxs_ in batch_idxs]
                batch_lengths = batch_lengths.to(DEVICE)
                batch_labels = batch_labels.to(DEVICE)
                # forward
                model.train()
                loss = model(batch_idxs, batch_lengths.cpu(), targets=batch_labels)
                ...
                if batch_id%10000==0:
                    ...
                    model.eval()
                    with torch.no_grad():
                        _, batch_predictions = model(batch_idxs, batch_lengths.cpu(), targets=batch_labels)
                    model.train()
                    batch_labels = batch_labels.cpu().detach().numpy()
                    batch_lengths = batch_lengths.cpu().detach().numpy()
            ...
            for batch_id, (batch_labels,batch_sentences) in enumerate(valid_data_iter):
                ...
                model.eval()
                with torch.no_grad():
                    batch_loss, batch_predictions = model(batch_idxs, batch_lengths.cpu() , targets=batch_labels)
                model.train()        
                valid_loss += batch_loss
                # compute accuracy in numpy
                batch_labels = batch_labels.cpu().detach().numpy()
