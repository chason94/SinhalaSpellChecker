...
from constants import sinhala_unicode
import torch
...
def get_char_tokens_sinhala(data):
    chars = list(""",;.!?:'\"/\\|_@#$%^&*~`+-=<>()[]{}""") + \
            list(sinhala_unicode.values())
...
def get_tokens(data,
    ...
    if topk is not None:
        sorted_ = sorted(token_freq.items(), key=lambda item: item[1], reverse=True)
        topk_lst = list(sorted_)[:topk]
        if len(topk_keep) and topk < len(token_freq.items()):
            total_keep_in = len(topk_keep)
            topk_keep = [(t, f) for (t, f) in token_freq.items() if t in topk_keep]
            print(f"Total tokens retained from keep: {len(topk_keep)} from {total_keep_in}")
            topk_lst = topk_keep + [(t, k) for (t, k) in list(sorted_) if t not in [t for t, k in topk_keep]][
                                   :topk - len(topk_keep) if topk > len(topk_keep) else 0]
        token_freq = {t: f for (t, f) in topk_lst}
        print(f"Total tokens retained : {len(token_freq.items())} ")
    ...
    if load_char_tokens:
        ...
        char_return_dict = get_char_tokens_sinhala("".join(
            [k for k, v in token2idx.items() if
             k not in [pad_token, unk_token, eos_token]]))  # get_char_tokens(use_default=False, data=data)
...
def sclstm_tokenize(batch_sentences, vocab, vector_type=VectorType.Default):
    """
...
def save_vocab_dict(path_: str, vocab_: dict):
    ...
    with open(path_, 'wb') as fp:
        pickle.dump(vocab_, fp, protocol=pickle.DEFAULT_PROTOCOL)
    return
