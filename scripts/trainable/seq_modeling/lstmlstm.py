...
if __name__=="__main__":
    ...
    if START_EPOCH>0:
        ...
        if FINETUNE!="":
            ...
            if TRAIN_NOISE_TYPE=="word":
                ...
                train_data = load_data(TRAIN_TEST_FILE_PATH, "train_06_jun_good.txt", "train_06_jun_bad.txt")
                valid_data = load_data(TRAIN_TEST_FILE_PATH, "test_06_jun_good.txt", "test_06_jun_bad.txt")
                print(len(train_data), len(valid_data))
            else:
                raise Exception("invalid TRAIN_NOISE_TYPE")
    ...
    vocab = load_vocab_dict(os.path.join(TRAIN_TEST_FILE_PATH, 'vocab.pkl'))
    save_vocab_dict(VOCAB_PATH, vocab)
    ...
    if START_EPOCH>0:
        ...
        for epoch_id in range(START_EPOCH,N_EPOCHS+1):
            ...
            for batch_id, (batch_labels,batch_sentences) in enumerate(train_data_iter):
                ...
                assert (batch_lengths_==batch_lengths).all()==True
                batch_idxs = [batch_idxs_.to(DEVICE) for batch_idxs_ in batch_idxs]
                batch_char_lengths = [batch_char_lengths_.cpu() for batch_char_lengths_ in batch_char_lengths] # .to(DEVICE)
                batch_lengths = batch_lengths.to(DEVICE)
                batch_labels = batch_labels.to(DEVICE)
                # forward
                model.train()
                loss = model(batch_idxs, batch_char_lengths, batch_lengths.cpu(), targets=batch_labels)
                ...
                if batch_id%5000==0:
                    ...
                    model.eval()
                    with torch.no_grad():
                        _, batch_predictions = model(batch_idxs, batch_char_lengths, batch_lengths.cpu(), targets=batch_labels)
                    model.train()
                    batch_labels = batch_labels.cpu().detach().numpy()
                    batch_lengths = batch_lengths.cpu().detach().numpy()
            ...
            for batch_id, (batch_labels,batch_sentences) in enumerate(valid_data_iter):
                ...
                assert (batch_lengths_==batch_lengths).all()==True
                batch_idxs = [batch_idxs_.to(DEVICE) for batch_idxs_ in batch_idxs]
                batch_char_lengths = [batch_char_lengths_.cpu() for batch_char_lengths_ in batch_char_lengths]
                batch_lengths = batch_lengths.to(DEVICE)
                batch_labels = batch_labels.to(DEVICE)
                # forward
                model.eval()
                with torch.no_grad():
                    batch_loss, batch_predictions = model(batch_idxs, batch_char_lengths, batch_lengths.cpu(), targets=batch_labels)
                model.train()        
                valid_loss += batch_loss
                # compute accuracy in numpy
                batch_labels = batch_labels.cpu().detach().numpy()
